...
public class HashTable<K extends Comparable<K>, D> implements Iterable<HashNode<K,D>>{
    ...
    private int hash(K key) {
        ...
        int h = key.hashCode();
        h = h ^ (h >>> 16);
        return (h & Integer.MAX_VALUE) % buckets.length;
    }
    ...
    private void internalPut(Bucket<K, D>[] target, K key, D data) {
        if (key == null) {
            target[0].put(null, data);
            return;
        }
        int h = key.hashCode();
        h = h ^ (h >>> 16);
        int index = (h & Integer.MAX_VALUE) % target.length;
    ...
    public List<D> toList() {
        List<D> list = new ArrayList<>(buckets.length);
        for (HashNode<K, D> node : this) {
    ...
    public List<K> keySet() {
        List<K> keys = new ArrayList<>(this.count);
        for (HashNode<K, D> node : this) {
    ...
    private interface Bucket<K extends Comparable<K>, D> {
        D put(K key, D data);
        D get(K key);
        D remove(K key);
        int size();
    ...
    private class ListBucket implements Bucket<K, D> {
        private LinkedList<HashNode<K, D>> list = new LinkedList<>();
    ...
    private class TreeBucket implements Bucket<K, D> {
        ...
        public D put(K key, D data) {
            ...
            HashNode<K, D> oldNode = tree.addOrModify(newNode);
