...
public class AVL <T extends Comparable<T>> implements Iterable<T> {
    ...
    private Node<T> remove(Node<T> current, T data, AtomicReference<T> removed) {
        ...
        if (cmp < 0) {
            ...
            T predecessorData = maxVal(current.getLeft());
            current.setData(predecessorData);
            current.setLeft(
                    remove(current.getLeft(),
                            predecessorData, new AtomicReference<>()
                    )
            );
        }
        updateMetadata(current);
    ...
    public T removeFirst() {
        ...
        AtomicReference<T> removedData = new AtomicReference<>();
        root = removeFirst(root, removedData);
        return removedData.get();
    }

    private Node<T> removeFirst(Node<T> current, AtomicReference<T> removed) {
        ...
        current.setLeft(removeFirst(current.getLeft(), removed));
        updateMetadata(current);
        return balance(current);
    }
    ...
    public T addOrModify(T data) {
        AtomicReference<T> oldData = new AtomicReference();
        root = addOrModify(root, data, oldData);
        return oldData.get();
    }

    private Node<T> addOrModify(Node<T> current, T data, AtomicReference<T> oldData) {
        if (current == null) return new Node<>(data);

        int cmp = data.compareTo(current.getData());

        if (cmp < 0) current.setLeft(addOrModify(current.getLeft(), data, oldData));
        if (cmp > 0) current.setRight(addOrModify(current.getRight(), data, oldData));
        else {
            oldData.set(current.getData());
    ...
    private T maxVal(Node<T> current) {
        if (current.getRight() == null) {
            return current.getData();
        }
        return maxVal(current.getRight());
    }
    ...
    public <R> List<R> map(Function<? super T, ? extends R> mapper) {
        ...
        for (T e : this) list.add(mapper.apply(e));
        return list;
    }

    public <R> LinkedList<R> mapLinkedList(Function<? super T, ? extends R> mapper) {
        Objects.requireNonNull(mapper);
    ...
    public void traverse(Consumer<? super T> action, TreeOrder order) {
        traverse(root, action, order);
    }

    private void traverse(Node<T> node, Consumer<? super T> action, TreeOrder order) {
        if (node == null) return;
        if (order == TreeOrder.PRE_ORDER) applyAction(node, action);
        traverse(node.getLeft(), action, order);
        if (order == TreeOrder.IN_ORDER) applyAction(node, action);
        traverse(node.getRight(), action, order);
        if (order == TreeOrder.POST_ORDER) applyAction(node, action);
    }

    private void applyAction(Node<T> node, Consumer<? super T> action) {
        for (int i = 0; i < node.getCount(); i++) {
            action.accept(node.getData());
        }
    }
    ...
    public T reduce(T identity, BinaryOperator<T> accumulator) {
        T result = identity;
        for (T e : this) result = accumulator.apply(result, e);
        return result;
    }
