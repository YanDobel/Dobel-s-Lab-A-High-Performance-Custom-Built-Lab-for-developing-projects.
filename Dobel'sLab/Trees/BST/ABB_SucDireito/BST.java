package Trees.BST.ABB_SucDireito;
...
public class BST <T extends Comparable<T>> implements Iterable<T> {
    ...
    public void addPorTamanho(T data) {
        this.root = addPorTamanho(root, data);
    }

    private Node<T> addPorTamanho(Node<T> current, T data) {
        if (current == null) return new Node<>(data);

        int novoTam = ((String) data).length();
        int atualTam = ((String) current.getData()).length();

        if (novoTam > atualTam) {
            current.setLeft(addPorTamanho(current.getLeft(), data));
        } else {
            current.setRight(addPorTamanho(current.getRight(), data));
        }
        return current;
    }

    public void reconstruir(String prefixo, String infixo) {
        this.preIndice = 0;
        this.root = construirRecursivo(prefixo, infixo, 0, infixo.length() - 1);
    }

    @SuppressWarnings("unchecked")
    private Node<T> construirRecursivo(String prefixo, String infixo, int inInicio, int inFim) {
        if (inInicio > inFim) return null;
        T data = (T) (Character) prefixo.charAt(preIndice++);

        Node<T> novoNo = new Node<>(data);
        if (inInicio == inFim) return novoNo;

        int inIndice = infixo.indexOf(data.toString());
        novoNo.setLeft(construirRecursivo(prefixo, infixo, inInicio, inIndice - 1));
        novoNo.setRight(construirRecursivo(prefixo, infixo, inIndice + 1, inFim));
        return novoNo;
    }
