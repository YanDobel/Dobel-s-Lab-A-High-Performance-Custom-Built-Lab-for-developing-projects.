...
public class Queue<T> implements Iterable<T> {
    ...
    public <R> Queue<R> map(Function<? super T, ? extends R> mapper) {
        Objects.requireNonNull(mapper);
        Queue<R> newQueue = new Queue<>(this.capacity);
        for (T val : this) {
            newQueue.enqueue(mapper.apply(val));
        }
        return newQueue;
    }
    ...
    public Queue<T> filter(Predicate<? super T> predicate) {
        ...
        Queue<T> filteredQueue = new Queue<>(this.capacity);
        for (T val : this) {
            if (predicate.test(val)) {
                filteredQueue.enqueue(val);
            }
        }
