...
public class AVL <T extends Comparable<T>> implements Iterable<T> {
    ...
    public T put(T data) {
        ...
        root = put(root, data, oldData);
        return oldData.get();
    }

    private Node<T> put(Node<T> current, T data, AtomicReference<T> oldData) {
        if (current == null) return new Node<>(data);

        int cmp = data.compareTo(current.getData());

        if (cmp < 0) current.setLeft(put(current.getLeft(), data, oldData));
        if (cmp > 0) current.setRight(put(current.getRight(), data, oldData));
        else {
